...
def get_message(message_id):
    try:
        server_client = StreamChat(api_key=settings.GETSTREAM_API_KEY, api_secret=settings.GETSTREAM_API_KEY)
        res = server_client.get_message(message_id)
        if res.status_code() not in [200, 201]:
            log(f"GetStaream Error: {res}", "get_message")
            log(f"Cannot get message: {message_id}", "get_message")
            return None
...
# print(get_message("753b66cd-5fcc-4702-8bb7-335cb154945a"))
